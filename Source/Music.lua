...
-- Fune experiment integration -------------------------------------------------
-- Keep the corrected legacy implementation above intact. FuneBridge wraps the
-- two entry points already used everywhere in Yacht (flipState and Refresh),
-- so GlobalBar, A+B, and simulator keyboard shortcuts do not need separate
-- playback code paths.
